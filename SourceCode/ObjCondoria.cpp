...
CObjCondoria::CObjCondoria(float x, float y)
{
	m_x = x;	//位置
	m_y = y;
}
//イニシャライズ
void CObjCondoria::Init()
{
	m_vx = 0.0f;	//ベクトル
	m_vy = 0.0f;
	...
	m_breakable_block_hit_test = false;//破壊可能ブロック判定無し

	//当たり判定用のHitBoxを作成
	Hits::SetHitBox(this,m_x,m_y,CONDORIA_SIZE, CONDORIA_SIZE,ELEMENT_ENEMY,OBJ_CONDORIA,1);
...
void CObjCondoria::Action()
{
	...
	//ダイクストラ法
	m_node = Dijkstra(m_x + block->GetScroll(),m_y,hero_x,hero_y,false,m_breakable_block_hit_test);
	...
	else
	{
		...
		else if(m_death_time == 80)
		{
			...
			((UserData*)Save::GetData())->mScore += CONDORIA_SCORE;

			//スコアオブジェクト作成
			CObjScore* obj_score = new CObjScore(CONDORIA_SCORE,m_x, m_y);
...
void CObjCondoria::Draw()
{
	...
	//表示位置の設定
	dst.m_top	= 0.0f + m_y;
	dst.m_left	= (CONDORIA_SIZE * m_posture )+ m_x + block->GetScroll();
	dst.m_right	= (CONDORIA_SIZE - CONDORIA_SIZE * m_posture )+ m_x + block->GetScroll();
	dst.m_bottom= CONDORIA_SIZE + m_y;
