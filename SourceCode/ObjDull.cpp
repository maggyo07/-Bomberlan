...
CObjDull::CObjDull(float x, float y)
{
	m_x = x;	//位置
	m_y = y;
}
...
void CObjDull::Init()
{
	...
	//当たり判定用のHitBoxを作成
	Hits::SetHitBox(this,m_x,m_y,DULL_SIZE, DULL_SIZE,ELEMENT_ENEMY,OBJ_DULL,1);
...
void CObjDull::Action()
{
	...
	//４ブロック分進んだら
	if (m_time >= BLOCK_SIZE / m_speed_power * 4)
	{
		//移動できる方向にランダムで移動
	...
	else
	{
		...
		else if(m_death_time == 80)
		{
			...
			((UserData*)Save::GetData())->mScore += DULL_SCORE;

			//スコアオブジェクト作成
			CObjScore* obj_score = new CObjScore(DULL_SCORE,m_x, m_y);
...
void CObjDull::Draw()
{
	...
	//表示位置の設定
	dst.m_top	= 0.0f + m_y;
	dst.m_left	= (DULL_SIZE * m_posture )+ m_x + block->GetScroll();
	dst.m_right	= (DULL_SIZE - DULL_SIZE * m_posture )+ m_x + block->GetScroll();
	dst.m_bottom= DULL_SIZE + m_y;
