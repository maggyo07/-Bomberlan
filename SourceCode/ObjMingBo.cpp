...
CObjMingBo::CObjMingBo(float x, float y)
{
	m_x = x;	//位置
	m_y = y;
}
//イニシャライズ
void CObjMingBo::Init()
{
	m_vx = 0.0f;	//ベクトル
	m_vy = 0.0f;
	...
	//当たり判定用のHitBoxを作成
	Hits::SetHitBox(this,m_x,m_y,MING_BO_SIZE, MING_BO_SIZE,ELEMENT_ENEMY,OBJ_MING_BO,1);

	element_number_x = 0;
	element_number_y = 0;

	m_time = BLOCK_SIZE / m_speed_power * 6;
}
//アクション
void CObjMingBo::Action()
{
	//ブロック情報を持ってくる
	...
	//４ブロック分進んだら
	if (m_time >= BLOCK_SIZE / m_speed_power * 6)
	{
		//移動できる方向にランダムで移動
	...
	else
	{
		...
		else if(m_death_time == 80)
		{
			...
			((UserData*)Save::GetData())->mScore += MING_BO_SCORE;

			//スコアオブジェクト作成
			CObjScore* obj_score = new CObjScore(MING_BO_SCORE,m_x, m_y);
...
void CObjMingBo::Draw()
{
	...
	//表示位置の設定
	dst.m_top	= 0.0f + m_y;
	dst.m_left	= (MING_BO_SIZE * m_posture )+ m_x + block->GetScroll();
	dst.m_right	= (MING_BO_SIZE - MING_BO_SIZE * m_posture )+ m_x + block->GetScroll();
